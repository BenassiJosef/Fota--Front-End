...
export class NewhireComponent implements OnInit {
  ...
  createHire(){
    ...
    this.hireservice.createHire(this.hire).subscribe( data => {return true},suc => {this.router.navigate(['/items']) }); error => {console.log("Error saving hire"); return Observable.throw(error)};
  }
